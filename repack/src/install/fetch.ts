...
import { FetchFailedException }        from './exceptions'
import { UnsupportedBindgenException } from './exceptions'
...
export async function fetchWasmBindgenAsset() {
	...
	switch (platform()) {
		case 'win32':
			assetRelease = 'pc-windows-msvc'
			break
		case 'linux':
			assetRelease = 'unknown-linux-musl'
			break
		case 'darwin':
			assetRelease = 'apple-darwin'
	}
...
export async function fetchBinaryenAsset() {
	...
	const target = assets.find(
		({ name }) => name.includes(assetArch) && name.includes(assetRelease) && !name.includes('sha256'),
	)
